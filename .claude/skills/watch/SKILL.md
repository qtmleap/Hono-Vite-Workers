---
name: watch
description: Monitor GitHub Actions CI for the current PR/branch until it concludes, then report green/red with failure logs. Use when the user says "watch the CI", "is CI passing?", "monitor the checks", or after opening a PR and wanting to wait for checks. Pairs with ship (before) and merge (after).
---
...
```
mcp__github__pull_request_read(method:'get_check_runs', owner:'qtmleap',
  repo:'Hono-Vite-Workers', pullNumber:<pr>)
```
...
- A common red here is commitlint: subject started with a capitalized English word, or
  `type` was `chore` instead of the repo's `chroe`. Fix the commit and re-push.
